...
public class VideoRecordThread extends Thread {
    ...
    public VideoRecordThread(MediaMuxerThread mutex, int width, int height) {
        this.mMutex = new WeakReference<MediaMuxerThread>(mutex);
        this.width = width;
        this.height = height;
    ...
    public synchronized void frame(byte[] data) {
        if (isRecording) {
                ...
                while (outputBufferIndex >= 0) {
                    ...
                    if (bufferInfo.size > 0) {
                        ...
                        if (mediaMuxer != null) {
                            ...
                            mediaMuxer.addMutexData(new MutexBean(outData, bufferInfo, true));
                        }
                    }
    ...
    private static void NV21toI420SemiPlanar(byte[] nv21bytes, byte[] i420bytes, int width, int height) {
        Log.e("TAG", "nv21 length:" + nv21bytes.length + ",i420bytes length:" + i420bytes.length + ", width * height: " + width * height);
